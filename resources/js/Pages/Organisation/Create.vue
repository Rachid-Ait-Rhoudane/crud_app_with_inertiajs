...
<template>
    ...
    <form @submit.prevent="formInputs.post('/organisations')" class="my-16 p-8 bg-white grid grid-cols-1 lg:grid-cols-2 gap-6 rounded-md shadow-lg">
        ...
        <div class="space-y-2">
            <FormInput @update-value="(val) => formInputs.name = val" label="name" placeHolder="Organisation name" input-type="text">{{ formInputs.errors.name }}</FormInput>
            <FormValidationError v-if="formInputs.errors.name">{{ formInputs.errors.name }}</FormValidationError>
        </div>

        <div class="space-y-2">
            <FormInput @update-value="(val) => formInputs.email = val" label="email" placeHolder="Organisation email" input-type="email">{{ formInputs.errors.email }}</FormInput>
            <FormValidationError v-if="formInputs.errors.email">{{ formInputs.errors.email }}</FormValidationError>
        </div>

        <div class="space-y-2">
            <FormInput @update-value="(val) => formInputs.address = val" label="address" placeHolder="Organisation address" input-type="text">{{ formInputs.errors.address }}</FormInput>
            <FormValidationError v-if="formInputs.errors.address">{{ formInputs.errors.address }}</FormValidationError>
        </div>

        <div class="space-y-2">
            <FormInput @update-value="(val) => formInputs.city = val" label="city" placeHolder="Organisation city" input-type="text">{{ formInputs.errors.city }}</FormInput>
            <FormValidationError v-if="formInputs.errors.city">{{ formInputs.errors.city }}</FormValidationError>
        </div>

        <div class="space-y-2">
            <FormInput @update-value="(val) => formInputs.phone = val" label="phone" placeHolder="Organisation phone" input-type="text">{{ formInputs.errors.phone }}</FormInput>
